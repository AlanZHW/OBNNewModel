...
{
    ui->setupUi(this);
    this->setWindowTitle(tr("观测系统"));
    m_menuManger   = new LMGRMenuManager(this);
    m_toolManager  = new LMGRToolManager(this);
    m_AreaDataInfo = 0;
    m_AreaDataInfo     = 0;
    m_AreaListDlg      = 0;
    m_statusBar        = 0;
...
void LineManager::setCurrentProject(const QString& _projectPath, const QString& _projectName)
{
    m_projInfo.ProjectPath = _projectPath;
    m_projInfo.ProjectName = _projectName;
    /// ======
    m_projPath = _projectPath;
    m_projName = _projectName;
    /// ======
    m_depolySystem->setProjInfo(m_projInfo);
    m_RNDepolyments->setProjInfo(m_projInfo);
    /// ======
...
{
#if 0
    qDebug() << __FUNCTION__ << "\t" << __LINE__ << "\t areaName = " << areaName;
    if(!m_AreaDataInfo)
    {
...
{
#if 0
    qDebug() << __FILE__ << "\t" << __LINE__ << "\t" << __FUNCTION__ << "\t slotUpdateArea";
    //1  更新工区-------------
...
void LineManager::slotOpenArea(const QString &areaName)
{
    ...
    {
        QMessageBox::warning(this,"warning", QString("Area %1 has Opened").arg(areaName));
        return;
    }
    /// ====== 读取工区数据
    ...
    if(ok != 0)
    {
        QMessageBox::warning(this,"warning", QString("Read data failed."));
        return;
    }
    setCurrentAreaData(areaName,areaDataInfo);
}
...
{
    qDebug() << __FILE__ << "\t" << __LINE__ << "\t" << __FUNCTION__ << "\t areaName = " << areaName;
    ...
    /// ====== 更新统计信息
    m_StationsInfoWg->setRNInfoNum(m_RNDepolyments->depolyedCount(), m_RNDepolyments->pickedupCount());
    /// ====== 扫描当前工区的设备
...
void LineManager::setCurrentArea(const QString &areaName)
{
    ...
    //设置窗口标题
    this->setWindowTitle("观测系统-" + m_currentArea);
}
...
void LineManager::slotCloseCurrent()
{
    ...
    if(m_AreaDataInfo)
    {
        m_AreaView->setAreaData(0); ///< 图像区域数据清0
        delete m_AreaDataInfo;
        m_AreaDataInfo = 0;
    }
    if(m_RNDepolyments)
...
void LineManager::slotAssignRNDeployments()
{
    if(!m_AssignNodeDlg)
    {
        ...
        //投放更新
        connect(m_AssignNodeDlg,SIGNAL(signalUpdateAssignedDev(QString)),this,SLOT(slotUpdateCurrent(QString)));
    }
