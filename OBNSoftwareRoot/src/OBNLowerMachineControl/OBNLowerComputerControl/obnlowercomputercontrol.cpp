...
OBNLowerComputerControl::OBNLowerComputerControl(QWidget *parent)
    : QMainWindow(parent),
      m_obnSeatchHosts(NULL),
      n_selAllCheckBox(NULL)
    , ui(new Ui::OBNLowerComputerControl)
{
    ...
    connect(ui->widgetControl,  &OBNRealControlWidget::signalCurrentTabChange, this, &OBNLowerComputerControl::slotCurrentTabChangeRoot);
    /// ======
    QString nQSSInform;
...
void OBNLowerComputerControl::slotCurrentTabChangeRoot(const int & index)
{
    switch(index)
    {
    case 0:
    case 1:
    case 2:
        ui->widgetNodeList->show();
        break;
    case 3:
        ui->widgetNodeList->hide();
        break;
    }
}
...
void OBNLowerComputerControl::slotSearch()
{
    if(NULL == m_obnSeatchHosts)
    {
        ...
        connect(m_obnSeatchHosts, &ONBRetrieveAvailableNodes::signalCurrentHostsSend, this, &OBNLowerComputerControl::slotSearchHostsList);
    }
...
void OBNLowerComputerControl::slotSelectall(int)
{
    if(NULL != n_selAllCheckBox)
    {
        ...
        ui->widgetControl->selectAllFunction(flag);
    }
}
