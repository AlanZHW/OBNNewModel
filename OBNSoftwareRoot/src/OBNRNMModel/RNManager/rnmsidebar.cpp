...
FiltersBox::FiltersBox(QWidget *parent)
    :QGroupBox(parent)
{
    this->setTitle(tr("显示筛选器"));    /// Display Filters
    initBox();
}
...
void FiltersBox::initBox()
{
    ...
    reportBtn = new QPushButton(tr("确定"));    /// Report
    ...
    //download
    QRadioButton  *downloadBtn = new QRadioButton(tr("已下载"));  /// Downloaded
    mainLayout->addWidget(downloadBtn);
    groupBox->addButton(downloadBtn,DownLoad);

    QRadioButton* tameButton = new QRadioButton(tr("原子钟驯服"));
    mainLayout->addWidget(tameButton);
    groupBox->addButton(tameButton,Tame);
    ...
    ui(new Ui::RNMSideBar),m_limitDialog(dlg)
{
    ...
    connect(m_limitDialog,SIGNAL(limitsChanged(const QVector<LimitData>&)),m_legendBox,SLOT(limitChanged()));
