...
void LmgrAreaView::setAreaData(AreaDataInfo *data)
{
    ...
    m_scene->clear();//清除所有项
    m_legendItem = 0;
    qDebug() << __FUNCTION__ << "\t" << __LINE__ << "\t m_DataInfo = " << m_DataInfo;
    if(m_DataInfo == 0)
    {
    ...
    m_sceneRectF  = rect.adjusted(-Scene_Rect_Margin,-Scene_Rect_Margin,Scene_Rect_Margin, Scene_Rect_Margin);
    ...
    //添加炮站点-----------------
    m_shotNum = addStations(m_DataInfo->shotStationInfo,m_colorMap.value(Shot_Color),m_shotItems,m_shotTextItems);
    //添加接收站点-------------------
    m_recvNum = addStations(m_DataInfo->recvStationInfo,m_colorMap.value(Recv_Color),m_recvItems,m_recvTextItems);

    addLineNumber(m_DataInfo->recvStationInfo,m_colorMap.value(Recv_Line_Color),m_recvLineItems);

    addLineNumber(m_DataInfo->shotStationInfo,m_colorMap.value(Shot_Line_Color),m_shotLineItems);
...
qDebug() << __FILE__ << "\t lineNum = " << lineNum;
    for(int i= 0;i<lineNum;i++)
    {
        ...
        for(int j = 0; j<staionNum;j++)
        {
            ...
            if(j%step==0)
            {
                QGraphicsSimpleTextItem *textItem = new QGraphicsSimpleTextItem(QString::number(stations[j].sp));
