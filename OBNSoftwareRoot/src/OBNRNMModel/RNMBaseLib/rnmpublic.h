...
#define All_Update           (State_Update|Ftp_Update) //两者都更新  0x03
/// 更新容量
#define Ftp_Update_Memory    (0x04|Ftp_Update|State_Update) ///< 0x07
/// 更新下载进度
#define Ftp_Update_Progress  (0x08|Ftp_Update)              ///< 0x0a
/// 更新采样率
#define Ftp_Update_SampleFrequency  (0x18|Ftp_Update)       ///< 0x1a
///
#define Ftp_Update_Note             (0x20|Ftp_Update)   ///< 0x22

//---------------------------------------------------------------
//节点设备配置文件
#define Node_NODAL_FILE      "obncfg.lst"
...
public:
    ...
    /// ====== 俯仰角
    float isPitchAngle() const {return nodeData.pitchAngle;}
    void  setPitchAngle(const float & _pitchAngle) { nodeData.pitchAngle = _pitchAngle;}
    /// ====== 翻滚角
    float isRollAngle() const {return nodeData.rollAngle;}
    void  setRollAngle(const float & _rollAngle) { nodeData.rollAngle = _rollAngle;}
    /// ====== 方位角
    float isAzimuthAngle() const {return nodeData.azimuthAngle;}
    void  setAzimuthAngle(const float & _azimuthAngle) { nodeData.azimuthAngle = _azimuthAngle;}
    ...
    /// ====== 原子钟驯服相关 0:未驯服 1:驯服成功 -1驯服失败 2:正在驯服
    int  currentTameState(){return nodeData.tameState;}
    void setTameState(const int _tameState){nodeData.tameState = _tameState;}
...
private:
    struct NodeData
    {
        ...
        float     rollAngle;      // 翻滚角
        float     azimuthAngle;   // 方位角
    };
    //节点数据
    NodeData        nodeData;
    State           m_state[8];//参数状态
    int             m_id;
    NodeItem        *nodeItem;
