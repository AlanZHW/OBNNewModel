...
public:
   ...
   static QString projName(const ProjectInfo &);
    ...
    //检查项目文件，Id是否正确
    static int checkProject(const ProjectInfo &projInfo, QString& errorStr);
    //查询Project 信息
    static QJsonValue getProjectInfo(const ProjectInfo &,const QString &key,QString& errorStr,int *ok = 0);
    //修改Project 信息
...
public:
    ...
    static int updateArea(const ProjectInfo &,const QString &areaName, const QStringList &spsFiles,int option);
    static int updateReaplceArea(const ProjectInfo &,const QString &areaName,const QStringList &spsFiles);
    static int updateMergeArea(const ProjectInfo &,const QString &areaName,const QStringList &spsFiles);
    static void calShotLineInfo(const QStringList &lineList,QMap<QString,int> &shotLineInfo);
    static bool getAreaNames(const ProjectInfo&, QStringList&);
    /// ====== 项目添加工区
