...
void DeploySystem::slotSocketReadyRead()
{
    ...
    ui->textBrowser->append(QString("Device G%1 IP:%2").arg(devNo).arg(ipaddress));
    bool ok = addOrUpdateRecord(m_currentArea,device,ipaddress,m_workStatus);
    if(ok)
    {
        ui->textBrowser->append(QString("Add/Update Device %1 Success!").arg(device));
        emit signalUpdateDevice(m_currentArea);
    }
    else
    {
        ui->textBrowser->append(QString("Add/Update Device %1 Failed,Error:%2").arg(device).arg(m_errString));
    }
}
...
{
    QString profile = m_projInfo.ProjectPath +Dir_Separator+m_projInfo.ProjectName+Project_File_Suffix;
...
bool DeploySystem::addOrUpdateRecord(QString areaName,QString device, QString ipaddress,WorkStatus workStatus)
{
#if 0
