...
void QCGraphBase::resizeEvent(QResizeEvent *e)
{
    ...
    m_paintAction = DRAW;
    //更新图像区范围------------------
    m_GraphRect = QRect(Left_Scale_Margin,Top_Scale_Margin, width()-Left_Scale_Margin-Right_Scale_Margin, height()-Top_Scale_Margin-Bottom_Scale_Margin);
}
...
void QCGraphBase::paintEvent(QPaintEvent *e)
{
    ...
    if(NULL == m_gatherInfo)
    {
        return;
    }

    //是否有数据
    if(m_gatherInfo->NTraces < 1 || NULL == m_data)
    {
        return;
    }
