...
{
    qDebug()<<QString("Start produce shot oriented segy file,at %1").arg(QDateTime::currentDateTime().toString(DateTime_Format_Sec));
    ...
    for(int lineNo = startLine;lineNo<=lastLine;lineNo++)
    {
        ...
        qDebug()<<QString("%1% Process ShotLine %2,Shot: %3-%4.").arg(100*line/lineNum).arg(staLine.line).arg(startStation).arg(end);
    ...
    qDebug()<<QString("100% Data Process Finished , at %1.").arg(QDateTime::currentDateTime().toString(DateTime_Format_Sec));
    return true;
}
...
bool ShotOrientedSegy::closeSegyFiles()
{
    ...
    for(int i =0;i<4;i++)
    {
        ...
        if(ok!=0)
        {
            m_errString = QString("Close Ouputfile %1 failed.").arg(m_segyFileHandle->fileHandles[i].fileName);
            return false;
        }
    }
    return true;
...
bool ShotOrientedSegy::readRecvFiles()
{
    ...
    qDebug() << "dataPath = " << m_Parameter.dataPath;
    ...
    qDebug()<<"readed recvfiles,bhx files:"<<m_recvFilesList->bhxRecvFiles.size()
            <<"bhy files:"<<m_recvFilesList->bhyRecvFiles.size() << "bhz files:"<<m_recvFilesList->bhzRecvFiles.size()
...
bool ShotOrientedSegy::allocInitMemory()
{
    ...
    int mode = Read_Only;
    int ok = -1;
    OpenTrace *openTr = gobs_open_segy_func(m_recvFilesList->bhxRecvFiles[0]->fileName.toLatin1(),&mode,binary3200,binaryhead,segyInfo,&ok);
    if(ok!=0)
    {
                                    ...
                                    ,const int &handle,long long *curTrace)
{
    ...
    while(1)
    {
        gobs_read_segy_func(openTr,*curTrace,1,m_traceHead,m_data,&readedTr,&ok);
        //没有数据直接返回
        if(readedTr==0)
        {
            return 1;
        ...
        {
            ++*curTrace;
            gobs_write_segy_func(m_segyFileHandle->fileHandles[handle].openTrace,m_traceHead,m_data,&ok);
            if(ok!=0)
            {
                m_errString = QString("Write Outputfile failed.").arg(m_segyFileHandle->fileHandles[handle].fileName);
                return -3;
            }
            break;
        }
