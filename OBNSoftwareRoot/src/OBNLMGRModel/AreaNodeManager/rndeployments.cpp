...
//    SqlFunctions *sqlFunc = SqlFunctions::instance();
    ...
    qDebug() << "fileName = " << fileName << "\t " << fileInfo.fileName();

    /// ====== 20221007-此线号获取方式需要根据新的文件格式进行调整
    int line_no = 1;/// lineNo(fileInfo.fileName());
    ...
    ResultInfo   devInfo;
    devInfo.fieldName = TABLE_FIELD_DEVICE; ///<0
    retInfos.append(devInfo);

    ResultInfo   ipInfo;
    ipInfo.fieldName = TABLE_FIELD_IP;      ///<1
    retInfos.append(ipInfo);

    ResultInfo   areaInfo;
    areaInfo.fieldName = TABLE_FIELD_AREA;  ///<2
    retInfos.append(areaInfo);

    ResultInfo   lineInfo;
    lineInfo.fieldName = TABLE_FIELD_LINE;  ///<3
    retInfos.append(lineInfo);

    ResultInfo   staInfo;
    staInfo.fieldName = TABLE_FIELD_STATION;///<4
    retInfos.append(staInfo);

    ResultInfo   spsxInfo;
    spsxInfo.fieldName = TABLE_FIELD_SPSX;  ///<5
    retInfos.append(spsxInfo);

    ResultInfo   spsyInfo;
    spsyInfo.fieldName = TABLE_FIELD_SPSY;  ///<6
    retInfos.append(spsyInfo);

    ResultInfo   spszInfo;
    spszInfo.fieldName = TABLE_FIELD_SPSZ;  ///<7
    retInfos.append(spszInfo);

    ResultInfo   axInfo;
    axInfo.fieldName = TABLE_FIELD_ACTUALX; ///<8
    retInfos.append(axInfo);

    ResultInfo   ayInfo;
    ayInfo.fieldName = TABLE_FIELD_ACTUALY; ///<9
    retInfos.append(ayInfo);

    ResultInfo   azInfo;
    azInfo.fieldName = TABLE_FIELD_ACTUALZ; ///<10
    retInfos.append(azInfo);

    ResultInfo   depolyedInfo;
    depolyedInfo.fieldName = TABLE_FIELD_DEPOLYED;  ///<11
    retInfos.append(depolyedInfo);

    ResultInfo   pickedInfo;
    pickedInfo.fieldName = TABLE_FIELD_PICKEDUP;    ///<12
    retInfos.append(pickedInfo);

    ResultInfo   downloadInfo;
    downloadInfo.fieldName = TABLE_FIELD_DOWNLOADED;///<13
    ...
    while(!in.atEnd())
    {
        ...
        QString ipaddress = QString("192.168.%1.%2").arg(ip3).arg(ip4);
...
qDebug() << __FILE__ << "\t" << __LINE__ << "\t left = " << left << "\t right = " << right;
