...
{
    int ok = 0;
    QJsonValue value = getProjectInfo(projInfo,Json_Key_Project_Indentity, errorStr, &ok);
    if(ok != 0)
    {
        return ok;
    }
...
//查询Project 信息
QJsonValue Project::getProjectInfo(const ProjectInfo &projInfo,const QString &key,QString& errorStr,int *ok)
{
...
int Area::createArea(const ProjectInfo &projInfo,const QString &areaName,const QStringList &spsFiles)
{
    ...
    qDebug()<< __LINE__ << "\t new rFile:" << newFile;
    ...
    //创建文件，目录等-------------------------------------
    QString areaFileName = areaFile+Dir_Separator+areaName+Area_File_Suffix;
    QFile file(areaFileName);
    if(!file.open(QIODevice::WriteOnly))
    {
        return -3;
    }
...
int Area::updateReaplceArea(const ProjectInfo &projInfo,const QString &areaName,const QStringList &spsFiles)
{
    QString projPath = projInfo.ProjectPath;
    //QString projName = projInfo.ProjectName;
...
bool Area::getAreaNames(const ProjectInfo& projInfo, QStringList& pAreas)
{
    QString projPath = projInfo.ProjectPath;
    QString projName = projInfo.ProjectName;
    ...
    int nAreaNum = areasArray.size();
    for(int iArray = 0; iArray < nAreaNum; iArray += 1)
    {
        QString jvalInform = areasArray[iArray].toString();
        pAreas.append(jvalInform);
    }
    return true;
}
...
{
    QStringList areaList;
    QString areaFile = projPath + Dir_Separator+areaName +Dir_Separator+areaName +Area_File_Suffix;
