...
bool AssignNodeDlg::initUnassignedDevices()
{
    ...
    if(!ok)
    {
        QMessageBox::critical(this,"error","Query device table failed,error:"+sqlFunc->errorString());
        return false;
    }
    //总的设备列表
...
bool AssignNodeDlg::depolyDevice(NodeDevice *device)
{
    for(int i = 0;i<ui->currentDepolyTable->rowCount();i++)
    {
        ...
        if(!ui->currentDepolyTable->item(i,1))
        {
            ...
            ui->unassignedCount->setText(QString::number(ui->currentDepolyTable->rowCount() - m_assignedDevices.size()));
    ...
    for(int i=0;i<ui->currentDepolyTable->rowCount();i++)
    {
                ...
                &&ui->currentDepolyTable->item(i,1)->isSelected())
        {
            ...
            else
            {
                ...
                {
                    if(m_assignedDevices[j]->Device == device && unDepolyDevice(m_assignedDevices[j]))
                    {
                        ui->currentDepolyTable->setItem(i,1,0);
