...
void RNFtpManager::setProjectInfo(const ProjectInfo &projInfo)
{
    ...
    int ok  = 0;
    QString errStr;
    QString dataPath = Project::getProjectInfo(projInfo,Json_Key_Project_Data_Path, errStr,&ok).toString();
    if(ok != 0)
    {
        QMessageBox::critical(this,"Error",QString("Get Data Path Error,ok =%1").arg(ok));
    }
...
void RNFtpManager::finishedFtpWork(Node *node,const int &ftpWk,const bool &err)
{
    ...
    if(err)
    {
        ui->textBrowser->append(QString("G%1").arg(node->no(),3,10,QChar('0'))+" "+getWorkName(m_ftpWork)+" Error");
    }
...
void RNFtpManager::on_downloadBtn_clicked()
{
    ...
    if(ui->timePeriodCbx->isChecked())
    {
        ...
        if(fromTime>toTime)
        {
            QMessageBox::warning(this,"warning","Download from time is later than to time.");
            return;
        }
        if(ui->TClineEdit->text().isEmpty())
        {
            QMessageBox::warning(this,"warning","Input TC Value.");
            return;
        }
        qDebug()<<"start time:"<<fromTime.toString("yyyy-MM-dd HH:mm:ss")<<"to time:"<<toTime.toString("yyyy-MM-dd HH:mm:ss");
...
{
    //查找对应路径
    QString newPath = QFileDialog::getExistingDirectory(this,"Select Data Path",m_dataPath);
    if(newPath.isEmpty())
    {
        return;
    }
...
void RNFtpManager::on_checkBox_clicked(bool checked)
{
    ...
    for(int i = 0;i<m_tableModel->rowCount();i++)
    {
        if(m_nodes[i]->state())
        {
            m_sortModel->setData(m_sortModel->mapFromSource(m_tableModel->index(i,0)),checked);
        }
    }
}
